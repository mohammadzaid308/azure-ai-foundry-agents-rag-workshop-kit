...
def retrieve_context(data_dir: Path, question: str, top_k: int = 5) -> str:
    """IDF-weighted keyword retrieval.

    Plain token-overlap lets generic docs (the catalog, gift history, the system
    prompt) outrank the one profile the question is actually about, because
    common words like "gift"/"likes"/"dislikes" appear everywhere. Weighting each
    matched term by its inverse document frequency makes rare, discriminating
    terms (like a person's name) dominate, so the right profile is retrieved.
    """
    corpus = [(name, text, tokenize(text)) for name, text in load_corpus(data_dir)]
    ...
    scored = [
        (
            sum(math.log((total_docs + 1) / (doc_freq[t] + 1)) for t in q & tokens),
            name,
            text,
        )
        for name, text, tokens in corpus
    ]
